...
export default function LinkedInPage() {
  ...
  const startRun = useMutation({
    mutationFn: () =>
      startApifyRun({ job_titles: jobTitles, locations, max_items: maxItems, scraper_mode: scraperMode }),
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden">
      <div className="flex-1 flex overflow-hidden">
        {/* Left column: config */}
        <div className="w-72 flex-shrink-0 col-scroll p-4" style={{ borderRight: '1px solid var(--border)' }}>
          ...
          <div className="flex flex-col gap-3">
            ...
            {saveConfig.isSuccess && (
              <span className="text-xs" style={{ color: '#4ade80' }}>Config saved</span>
            )}
            {saveConfig.isError && (
              <span className="text-xs" style={{ color: '#f87171' }}>Save failed</span>
            )}
