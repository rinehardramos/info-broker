...
def list_jobs(user: dict = Depends(get_current_user)):
    rows = fetch_all(
        "SELECT id::text, status, query, created_at, completed_at FROM v3_jobs WHERE user_id = %s ORDER BY created_at DESC LIMIT 50",
        (str(user["id"]),),
    )
...
def get_job(job_id: str, user: dict = Depends(get_current_user)):
    row = fetch_one(
        "SELECT id::text, status, query, created_at, completed_at FROM v3_jobs WHERE id = %s AND user_id = %s",
        (job_id, str(user["id"])),
    )
    if not row:
